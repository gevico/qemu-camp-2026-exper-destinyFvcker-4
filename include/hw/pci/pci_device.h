 ...
 */
static inline MemTxResult pci_dma_rw(PCIDevice *dev, dma_addr_t addr, void *buf,
                                     dma_addr_t len, DMADirection dir,
                                     MemTxAttrs attrs) {
  return dma_memory_rw(pci_get_address_space(dev), addr, buf, len, dir, attrs);
}
...
#define PCI_DMA_DEFINE_LDST(_l, _s, _bits)                                     \
  static inline MemTxResult ld##_l##_pci_dma(PCIDevice *dev, dma_addr_t addr,  \
                                             uint##_bits##_t *val,             \
                                             MemTxAttrs attrs) {               \
    return ld##_l##_dma(pci_get_address_space(dev), addr, val, attrs);         \
  }                                                                            \
  static inline MemTxResult st##_s##_pci_dma(PCIDevice *dev, dma_addr_t addr,  \
                                             uint##_bits##_t val,              \
                                             MemTxAttrs attrs) {               \
